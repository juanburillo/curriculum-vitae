...
<template>
  <section class="flex flex-row gap-x-16">
    ...
    <div>
        ...
        <div>
          <h1 class="font-bold text-3xl text-gray-800">{{ data.name }}</h1>
          <h2 class="mb-2 font-medium text-gray-700">{{ data.title }}</h2>
          <p class="text-lg text-justify text-gray-700">{{ data.description }}</p>
        </div>
    </div>
  </section>
</template>
